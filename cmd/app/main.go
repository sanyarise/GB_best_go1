...
func main() {
	...
	logMsg := fmt.Sprintf("config : %v successfully import", s) // misspell
	logger.Debug(logMsg)
	if err != nil {
	...
	crCtx, canc := context.WithTimeout(context.Background(), time.Duration(cfg.ReqTimeout)*time.Second) // Таймаут парсера, получения ссылок, формирования заголовков
	defer canc()                                                                                        //
